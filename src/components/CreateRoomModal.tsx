...
export function CreateRoomModal({ onClose, initialMovieTitle }: CreateRoomModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (sourceTab === 'local' && localSubTab === 'download' && activeTorrentId !== null && !isComplete) {
            ...
            } else if (finalLocalPath && finalLocalPath !== youtubeUrl) {
                savePreScreenData(roomId as string, {
            ...
            <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '560px' }}>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="modal-footer">
                        ...
                        <button type="submit" className="btn btn-primary" disabled={!movieSelected || isLoading}>
                            {isLoading ? <span className="spinner" /> : (
                                <>
                                    <img src={logo} alt="" style={{ height: '1.2em', marginRight: '8px', verticalAlign: 'middle' }} />
                                    {sourceTab === 'local' && localSubTab === 'download' && isDownloading && !isComplete ? 'Create Room (Downloading...)' : 'Create Room'}
                                </>
                            )}
                        </button>
                    </div>
